# ihp-sg13cmos5l Ring Oscillator

This is the **three-stage ring oscillator with output buffer** of the [HeiChips 2026 Analog Workshop](../README.md), the reference solution of exercise 2: four instances of the `inverter` unit cell, three of them closed into a ring and the fourth used as output buffer, with its complete flow (schematic → simulation → layout → DRC/LVS/PEX). The exercise is described on the [workshop website](https://heichips.github.io/heichips26-analog-workshop/#sec-exercise2).

<p align="center">
  <a href="final/render/ringosc_white.png">
    <img src="final/render/ringosc_white.png" alt="Render of the ihp-sg13cmos5l ringosc layout" width=70%>
  </a>
  <br>
  <em>Render of the ihp-sg13cmos5l ringosc layout.</em>
</p>
...
│  │  ├─ *.ext.spc
│  │  ├─ ringosc_klayout.cir
│  │  └─ ringosc_magic.ext.spc
│  ├─ 📁 pex/
│  │  ├─ *.spice
│  │  ├─ ringosc_klayout_pex_*.spice
│  │  └─ ringosc_magic_pex_*.spice
│  └─ 📁 schematic/
│     ├─ *.cdl
│     ├─ *.spice
│     ├─ ringosc_klayout.cdl
...
All targets that operate on a specific cell accept an optional `CELL=<cellname>` parameter. The default is the top-level cell (`ringosc`).
...
```sh
make sim-xschem TB=ringosc_tb_tran
```
...
```sh
make sim-view-xschem SCRIPT=plot_ringosc
```
...
Renders the top-level layout GDS with `scripts/lay2img.py` and saves the two images `ringosc_black.png` and `ringosc_white.png` in `final/render/`:
...
```sh
make klayout-drc
make klayout-drc CELL=ringosc
make klayout-drc CELL=ringosc DRC_LEVEL=regular
```
...
make klayout-lvs-netlist CELL=ringosc
...
make magic-lvs-netlist CELL=ringosc
...
```sh
python3 scripts/check_pex_ports.py netlist/pex/ringosc_magic_pex_2.spice
...
```sh
make klayout-pex
make klayout-pex CELL=ringosc
make klayout-pex CELL=ringosc EXT_MODE=3
```
...
```sh
make magic-pex
make magic-pex CELL=ringosc
make magic-pex CELL=ringosc EXT_MODE=3
```
...
```sh
make magic-pex CELL=ringosc EXT_MODE=3 THRESHOLD=5000 MINRES=500 MINDELAY=2
```
...
```sh
make klayout-verify
make klayout-verify CELL=ringosc
```
...
```sh
make magic-verify
make magic-verify CELL=ringosc
```
...
> [!NOTE]
> The `plot_simulations/data/` and `plot_simulations/figures/` folders themselves are kept and only emptied, so the folder structure survives a clean. This matters for `data/` in particular: the testbenches write their results into it with `wrdata`, which fails if the folder does not exist when a simulation is started interactively from Xschem.
...
> [!NOTE]
> The Xschem testbenches `.include` the PEX netlist `netlist/pex/ringosc_magic_pex_3.spice`. Directly after `make clean`, run `make magic-pex` (or the full `make all`) once before `make sim-xschem` / `make sim-all`, otherwise the include fails.

## Start a New Macro from This Template

The macro is a good starting point for a new analog cell (the workshop exercises use it this way):

1. Copy this macro folder.
...
For a new macro named `amp`, this looks as follows:
...
for f in schematic/xschem/ringosc* testbenches/xschem/ringosc*; do
    mv "$f" "$(echo "$f" | sed 's/ringosc/amp/')"
done
mv layout/ringosc.gds layout/amp.gds
```

Finally, update the remaining `ringosc` references inside the renamed files with search-and-replace in a text editor, for example the `ringosc.sym` instances and the `.include` of the PEX netlist in the testbenches (Xschem files are plain text).
