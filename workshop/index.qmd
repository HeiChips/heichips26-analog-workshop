...
| Inversion level | $g_\mathrm{m}/I_\mathrm{D}$ ($\mathrm{S/A}$) | Character |
| :-------------- | :---: | :--------- |
| **Strong** | below $\approx 7.5$ | Large overdrive, high current density $I_\mathrm{D}/W$, so a given current needs little width. Fast, small, and area-efficient, but each ampere buys little $g_\mathrm{m}$. |
| **Moderate** | $\approx 7.5$ to $\approx 17.5$ | The compromise region. The speed-efficiency product $f_\mathrm{T} \cdot g_\mathrm{m}/I_\mathrm{D}$ peaks here, which is why most amplifier stages end up in it. |
| **Weak** | above $\approx 17.5$ | Maximum $g_\mathrm{m}$ per ampere, so lowest power and highest intrinsic gain, with small $V_\mathrm{ds,sat}$ and therefore good voltage headroom. The price is width, capacitance, and speed. |

: Inversion levels and what they mean. {#tbl-inversion}

The channel length works largely independently of this. Long $L$ raises the output resistance $r_\mathrm{ds}$ and thus the intrinsic gain, improves matching, and lowers flicker noise (which scales with $1/L^2$ at constant $g_\mathrm{m}/I_\mathrm{D}$). Short $L$ raises $f_\mathrm{T}$ and shrinks area and gate capacitance. Speed and gain therefore pull in opposite directions.

@fig-tradeoffs maps both knobs onto one picture. The arrows point from the center towards the corner where each design parameter reaches its **best** value, so the same parameter is at its worst in the opposite direction.
...
5. Open `amp.sch` in Xschem and add a resistor between the `vin` and `vout` nets, as shown in @fig-ex1. For a first schematic-level study, the generic `res.sym` device is fine. Start with $R_\mathrm{f} = 1\,\mathrm{M\Omega}$.
...
7. Vary $R_\mathrm{f}$ and observe the trade-off. Too small, and the resistor loads the amplifier output and reduces the gain. Too large, and the time constant with the input capacitance limits the low end of the signal band. A value in the $1$ to $10\,\mathrm{M\Omega}$ range is a sensible choice for this sizing.
8. Propagate the resistor to the layout (`layout/amp.gds`) using an `rppd` or `rhigh` PCell (also place dummy resistors next to it), and re-run the verification chain with `make klayout-verify` and `make magic-verify`.
