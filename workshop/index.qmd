...
| `make klayout-setup` | Install the KLayout productivity plugins into your user directory (see @sec-klayout-plugins) |
...
| `final/` | Build deliverables for top-level integration (GDS, LEF, LIB, Verilog stub) |
| `render/` | Layout render images for documentation |
...
| `make sim-view-xschem SCRIPT=plot_inverter` | Plot the exported simulation data with Python |
...
Most targets accept optional variables, for example `CELL=<cellname>` to select a specific cell (defaults to `inverter`), `EXT_MODE=<1|2|3>` for the PEX mode, and `DRC_LEVEL=<precheck|macro|regular>` for the KLayout DRC rule set. See the [inverter README](https://github.com/HeiChips/heichips26-analog-workshop/blob/main/inverter/README.md) for the full list.
...
The [KLayout Productivity Suite](https://github.com/iic-jku/klayout-productivity-suite) is a collection of plugins developed at JKU that make manual layout much faster. They were already installed with `make klayout-setup` in @sec-overview. The full documentation with animations is [here](https://iic-jku.github.io/klayout-productivity-suite/doc/doc.html). A short summary:
...
![Self-biased inverter amplifier with feedback resistor $R_\mathrm{f}$.](fig/exercise_selfbiased_amp.svg){#fig-ex1 width=55%}

Recommended steps:

1. Open [`inverter.sch`](https://github.com/HeiChips/heichips26-analog-workshop/blob/main/inverter/schematic/xschem/inverter.sch) in Xschem and add a resistor between the `vin` and `vout` nets, as shown in @fig-ex1. For a first schematic-level study, the generic `res.sym` device is fine. Start with $R_\mathrm{f} = 1\,\mathrm{M\Omega}$.
2. Think first, then simulate. What do you expect to happen to the dc transfer characteristic, the ac response, and the measured gain? Then re-run the three testbenches and check whether the results match your expectations. Note that the ac open-loop testbench drives `vin` directly with a source, so to observe the self-biasing effect, also try disconnecting the input bias and let the feedback set the operating point.
3. Vary $R_\mathrm{f}$ and observe the trade-off. Too small, and the resistor loads the amplifier output and reduces the gain. Too large, and the time constant with the input capacitance limits the low end of the signal band. A value in the $1$ to $10\,\mathrm{M\Omega}$ range is a sensible choice for this sizing.
4. [Optional] Propagate the resistor to the layout using an `rppd` or `rhigh` PCell (also place dummy resistors next to it), and re-run the verification chain with `make klayout-verify` and `make magic-verify`.
...
![Three-stage ring oscillator with the fourth inverter as output buffer.](fig/exercise_ring_oscillator.svg){#fig-ex2 width=75%}

Recommended steps:

1. Create a new schematic (for example `ringosc.sch`) and place four instances of `inverter.sym`. Connect three of them in a ring, and tap the ring node with the fourth inverter as the buffer. Add the pins `vout`, `VDD`, and `VSS`.
2. Create a symbol for the new cell (press `A`), following @sec-draw-symbol.
3. Create a transient testbench. Since the delay per stage is small, simulate a few hundred nanoseconds with a fine step. Measure the oscillation frequency, for example with two `meas tran ... when v(vout)=0.75 rise=...` commands that time consecutive rising crossings of $V_\mathrm{cm}$, and compute $f_\mathrm{osc} = 1/T$. The expected relation is $f_\mathrm{osc} = 1/(2 \cdot N \cdot t_\mathrm{d})$ with $N = 3$ stages and $t_\mathrm{d}$ the effective stage delay.
4. Think about the startup. An ideal simulation can rest on the metastable point where all stages sit at $V_\mathrm{cm}$. If the ring does not start, kick it with an initial condition, for example `.ic v(osc)=0`.
5. [Optional] Draw the layout with four instances of the `inverter` layout cell and route the ring, following @sec-draw-layout. Re-run DRC and LVS, extract the parasitics with `make magic-pex CELL=ringosc`, and compare the post-layout oscillation frequency against the schematic level. The parasitics add delay per stage, so the frequency drops.
